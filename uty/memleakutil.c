...
void processHeapwalk(int cmd, int pid, int tid, bool isSelfTest, LIST *resp, int *listIndex, MMAP_anon *mmapIn)
{
	...
	if (HEAPWALK_MMAP_ENTRIES == cmd) {
		...
		// This recursive call will complete HeapWalkAll processing
								...
								if (NULL == mmapIn) {
									if ((tid)?tid == msgresp.xfer[msgIndex].tid : 1) {
										...
										PRINT("%u %p %u %p %u %ld%s\n", ++msgSeq,
											msgresp.xfer[msgIndex].ptr, msgresp.xfer[msgIndex].size, msgresp.xfer[msgIndex].ra,
									...
									// JAGA add here for grouping //
									// update mmap entry
									// TODO consider memalign'd overhead using flags...
									...
									if (NULL == tmpprn) { // Cannot happen...even if the pointer is invalid, it's go into default one
